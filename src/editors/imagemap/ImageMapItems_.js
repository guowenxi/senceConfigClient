...
&::before{
	...
	background-color: rgba(11, 152, 236, 1);
}
`;
...
class ImageMapItems extends Component {
	...
	handlers = {
		...
		onDrawingItem: item => {
			...
			if (canvasRef.handler.interactionMode === 'polyLine') {
				message.info('Already drawing');
				return;
			}
	...
	renderItems = items => (
		<Flex flexWrap="wrap" flexDirection="row" style={{ width: '100%' }}>
	...
	render() {
		...
		return (
			<Box className='ImageMapItems' flex="1" flexDirection="column" style={{ height: '100%' }}>
					<Scrollbar>
						<Flex flex="1" 
						flexWrap="wrap" 
