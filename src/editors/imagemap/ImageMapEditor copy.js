...
import ImageMapItems from './ImageMapItems____';
...
class ImageMapEditor extends Component {
	...
	componentDidMount() {
		...
		axios.get("http://192.168.2.92:9000/control-module/deviceModule/getDeviceAllList").then((data)=>{
			this.handlers.onImportJson(data.data.data);
		})
		//是否默认加载
		//这里需要改下
		import('./Descriptors.json').then(descriptors => {
			axios.get("http://192.168.2.92:9000/control-module/deviceModule/getDeviceClassAndClass").then((data)=>{
				let _list = [];
				data.data.data.map((item,idx)=>{
					let _item = {};
					descriptors.map((item2,idx)=>{
						if(item.deviceClassTypeId == item2.typeId){
							_item = item2;
						}
					})
					_item.option.code = Object.assign(_item.option.code,item);
					_list.push({
						..._item,
	...
	canvasHandlers = {
		...
		onChange: (selectedItem, changedValues, allValues) => {
			console.log("ImageMapEditor.js--- 166")
			const { editing } = this.state;
			if (!editing) {
				this.changeEditing(true);
			}
