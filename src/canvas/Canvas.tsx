...
class InternalCanvas extends Component<CanvasProps, IState> implements CanvasInstance {
	...
	render() {
		...
		return (
			...
			>
				<div id="points" style={{position:"absolute",width:'4px',height:'4px',zIndex:100,background:"red"}}></div>
				<canvas id={`canvas_${id}`} className="echarts" style={{position:"absolute",width:'1px',height:'1px',zIndex:50}}/>
				<div id={`echarts`} style={{position:"absolute",width:'100%',height:'100%',zIndex:0,left:0,top:0}}>
				</div>
			</div>
		);
	}
