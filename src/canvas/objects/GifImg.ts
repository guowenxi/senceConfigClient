...
const GifImg = fabric.util.createClass(fabric.Rect, {
	type: 'gifimg',
	superType: 'element',
...
GifImg.fromObject = (options: ElementObject, callback: (obj: ElementObject) => any) => {
	return callback(new GifImg(options.code, options));
};

// @ts-ignore
window.fabric.GifImg = GifImg;
