...
const Select = fabric.util.createClass(fabric.Rect, {
	type: 'Select',
	superType: 'element',
...
Select.fromObject = (options: ElementObject, callback: (obj: ElementObject) => any) => {
	return callback(new Select(options.code, options));
};

// @ts-ignore
window.fabric.Select = Select;
